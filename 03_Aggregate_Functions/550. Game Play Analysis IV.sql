/* PROBLEM: 550. Game Play Analysis IV
DIFFICULTY: Medium
TOPIC: Aggregate Functions & Window Functions

LOGIC:
The objective is to calculate the Day-1 retention rate: the fraction of 
players who logged in again exactly one day after their first login. 
We use ROW_NUMBER() to identify the first login and LEAD() to find the 
interval until the subsequent login.

ARCHITECTURAL NOTE: 
1. Retention Logic: By combining ROW_NUMBER and LEAD, we create a precise 
   temporal map for each player. 'init_log = 1' ensures we only evaluate the 
   first login, while 'log_days = 1' identifies the success metric.
2. PostgreSQL Date Arithmetic: PostgreSQL simplifies date differences by 
   allowing direct subtraction (Date2 - Date1), which returns an integer 
   representing days. This is cleaner and more performant than SQL Server's 
   DATEDIFF function.
3. Ratio Calculation: We cast the final result to '::numeric' to satisfy 
   PostgreSQL's ROUND function requirements, and utilize NULLIF to prevent 
   'Division by Zero' in the event of an empty user base.
*/

WITH player_logs AS (
    SELECT
        player_id,
        -- Calculate days between current login and the next one
        LEAD(event_date) OVER(PARTITION BY player_id ORDER BY event_date) - event_date AS log_days,
        -- Identify the first login record
        ROW_NUMBER() OVER(PARTITION BY player_id ORDER BY event_date) AS init_log
    FROM activity
)
SELECT
    ROUND(
        (SUM(CASE WHEN init_log = 1 AND log_days = 1 THEN 1 ELSE 0 END) * 1.0 / 
        NULLIF(COUNT(DISTINCT player_id), 0))::numeric, 
    2) AS fraction
FROM player_logs;
